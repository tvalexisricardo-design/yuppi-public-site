...
import { getCombo } from "@/lib/professionals";
...
export async function generateMetadata({
  ...
  const combo = await getCombo(categoria, cidade);
  if (!combo) return {};

  const title = `${combo.categoriaNome} em ${combo.cidadeNome} | Yuppi`;
  const description = `Encontra ${combo.categoriaNome.toLowerCase()} para festas infantis em ${combo.cidadeNome}. Consulta profissionais e contacta-os diretamente.`;
      ...
      <section className="relative overflow-hidden px-6 pb-16 pt-16 md:pt-24">
        ...
        <div className="relative mx-auto max-w-content">
          ...
          <p className="mt-6 max-w-xl text-lg leading-relaxed text-inkSoft">
            Profissionais e negócios de {combo.categoriaNome.toLowerCase()} em {combo.cidadeNome}. Consulta os perfis e contacta diretamente cada profissional.
          </p>
        </div>
      </section>

      <section className="border-y border-black/5 bg-canvasSoft px-6 py-8">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {combo.perfis.map((p, i) => (
              <Reveal key={p.slug} delay={i * 80}>
                ...
                >
                  <div className="aspect-square w-full"><ProfileImage src={p.coverImage} alt={p.nome} className="h-full w-full object-cover" /></div>
